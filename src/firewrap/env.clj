...
(defn unset-var-args [env allowed-vars]
  (let [unallowed (-> (set/difference
                       (set (keys env))
                       allowed-vars)
                      sort)]
    (for [v unallowed]
...
(defn set-allowed-vars [ctx allowed-vars]
  ...
  (let [{:keys [getenvs]} ctx
        args (unset-var-args (getenvs) allowed-vars)]
    (apply system/add-bwrap-args ctx args)))

(comment
  (unset-var-args (System/getenv) allowed))
